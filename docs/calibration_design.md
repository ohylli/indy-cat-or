...
- **Calibration report — done (V0).** `scripts/calibration_report.py` measures and renders the textual report of §4, **V0 subset — distributions only, no threshold**: the positive/negative distribution table (`n mean min p50 p95 max`), the lowest-positive-vs-highest-negative overlap line, a **look-alike-vs-easy** breakdown (look-alike = the four long-haired breeds **Maine_Coon, Ragdoll, Birman, Persian**), per-breed negative scores sorted by max, and the highest-scoring-negatives / lowest-scoring-positives risk lists. `--scores-out` writes the per-image CSV joined with provenance. Covered by `tests/test_calibration_report.py`. Measured on the baseline split, the distributions separate clearly (Indy positives mean ≈0.83 vs Oxford negatives ≈0.26) with the long-haired breeds forming the high negative tail and a real overlap (a handful of Maine Coon / Ragdoll / Persian crops scoring above the weakest positive) — exactly the signal V0 exists to surface.
- **CLI — done for V0.** `scripts/calibrate.py` is the entry point (generation folded in, per §4): generation flags, a `--seed`/`--random-seed` group, `--manifest` replay (mutually exclusive with generation flags), `--generate-only`, plus the scoring options `--aggregation {max,mean-top3}` and `--scores-out`. Manifests are written under `data/splits/` (gitignored). A normal run generates + saves the manifest **and** scores it, printing the V0 report; the `--manifest` replay path scores too; `--generate-only` stops after writing the manifest. `test` is never read. Covered by `tests/test_calibrate.py`.
- **Not yet started:** §5's V1–V3 (threshold sweep / trade-off curve, automated policy pick, the frozen calibration artifact + `max`-vs-`mean-top3` comparison) and the separate `evaluate.py`. `leave_one_out` is unimplemented (the `strategy` string is stored; only `three_way` is generated).
