...
- **Not yet started:** the separate `evaluate.py` (reads `test` + a frozen artifact, reports honest numbers at the frozen threshold) — now **specified in full in §7** (inputs, the disjointness guard, the confusion-matrix/drift report, and the E0→E2 staging; the shared-rendering-primitives refactor it depends on is done, above), pending implementation. `leave_one_out` is unimplemented (the `strategy` string is stored; only `three_way` is generated).
