...
- **Not yet started:** `evaluate.py` **E1+** (the false-positive/false-negative error lists with embedded crops, and the `--scores-out` CSV). `leave_one_out` is unimplemented (the `strategy` string is stored; only `three_way` is generated).
