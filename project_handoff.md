...
For the decision step, the plan is to **start with k-Nearest Neighbors (kNN)** because it is simple and needs no real training, then **move to a linear probe** (a small classifier trained on top of the frozen embeddings) if kNN accuracy proves insufficient. Both operate on precomputed vectors and are cheap. The progression from kNN to linear probe is expected but should be driven by measured results rather than assumed.
...
**Positives (Indy).** Because the pretrained model does the heavy lifting, this is few-shot: roughly 10 to 20 varied photos is expected to be plenty, and fewer may work given how distinctive he is. Variety matters far more than count — different angles, distances, lighting, and especially different backgrounds — to avoid the model latching onto a shared background instead of the cat. Photos that show his head and tail clearly are the most useful.

**Negatives (other cats).** The plan is to start with the **Oxford-IIIT Pet dataset**: a well-labeled, openly licensed (CC-BY) set with on the order of a couple of thousand cat images across twelve breeds. Crucially, several of those breeds — Maine Coon, Ragdoll, Birman — are long-haired cats that resemble Indy, so they serve as built-in harder negatives, while the short-haired breeds give easy general coverage.

Later, if needed, add a dataset that includes **Norwegian Forest Cats** specifically (larger community breed collections on Kaggle or Hugging Face are the likely source), since same-breed cats are the most demanding negatives of all and the closest match to Indy's look.

A practical, accessibility-relevant note: prefer **pre-labeled datasets organized by breed**. The breed label is trustworthy text, so the set can be assembled and filtered without having to visually inspect each image — presence of a breed is a text check on folder or label names, not a visual one. Raw image-search sources are best avoided as a primary source for this reason. When assembling negatives, keep a reasonable spread across breeds rather than a pile of one, so the system learns "not Indy" rather than "not one particular breed."
...
To make this measurable, fix a **held-out test set up front and never use it during setup**. It should include some Indy photos and some other cats, and — importantly — several Norwegian Forest Cats that never appear in the gallery or negatives. That look-alike slice is the real exam.
...
The one structural principle worth holding to: keep the **core recognition logic separate from any user interface**. The embedding-and-classify logic should live in plain, importable Python modules that know nothing about how they are presented. Whatever UI sits on top — command line, Streamlit, or a custom web app — should be a thin layer that calls into that core.
...
- **Decision method** — kNN first; whether and when to adopt a linear probe should follow from measured accuracy, not be assumed.
...
- **UI** — Streamlit versus a custom accessible UI is to be decided by how Streamlit's accessibility actually feels in use.
