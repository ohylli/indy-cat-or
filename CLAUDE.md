...
- **Prediction app** — `scripts/predict_app/` is the deliverable "Is it Indy?" Streamlit UI (`uv run streamlit run scripts/predict_app/app.py`): upload a photo, get a text-first verdict (Indy / not Indy) with detection confidence, crop, closest gallery match, and the score-vs-threshold margin. `predict.py` is the streamlit-free core — `classify(...)` composes detect→crop→embed→score over one image and returns plain `ClassifyResult`/`CropResult` data (handles the no-cat and multiple-cat edge cases; carries each crop image back for display); `find_artifacts`/`build_gallery` wire the **frozen calibration artifact** (`data/artifacts/*.yaml`) into the live `Gallery` — never `data/embeddings/indy/`, so held-out test photos stay out of the live decision. `app.py` is the thin layer: `@st.cache_resource` model/artifact loading, an `st.form` gating compute, a threshold-override slider (default = frozen value), EXIF-correction of the upload (matches the upright gallery), and heading-led rendering with bare `st.image` (no static serving — that's only for the data-review grid). `tests/test_predict.py` covers the core model-free (fakes subclass `CatDetector`/`Embedder`). The detect toggle is the handoff-required measurable on/off.
