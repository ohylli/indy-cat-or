...
All four pipeline stages exist, both galleries (positives + negatives) are built, and the stage-1 baseline calibration + evaluation tooling is done. Two design docs hold the deep detail — consult them rather than duplicating it here: `docs/calibration_design.md` (the decide-stage tooling, CLI surface, manifest/artifact formats, and the V0→V3 / E0→E1 measured record) and `docs/embeddings_provenance.md` (the variant-nested cache layout and provenance rules).

- **Detect & crop** — `src/indycat/detection.py`: `CatDetector` takes an opened PIL image (callers own I/O); `detect_and_crop` composes the detect+crop pipeline prefix. **yolo11x is the default detector** (measured fewest misses on the 35 Indy photos). `scripts/detect_indy_gallery.py` is a textual sanity-check tool. Tests: `tests/test_detection.py`.
- **Embed** — `src/indycat/embedding.py`: `Embedder` wraps a frozen DINOv2 (base) via HF `transformers` and returns **raw, un-normalized** float32 vectors (normalization is deferred to decide); the backbone is swappable. `scripts/build_indy_gallery.py` builds the Indy gallery, detecting on the fly. Embedding caches are variant-nested under `data/embeddings/indy/<model>/<crop>/` with an `embeddings.meta.yaml` provenance sidecar (see `docs/embeddings_provenance.md`). Tests: `tests/test_embedding.py`.
- **Negatives gallery** — `scripts/build_oxford_negatives.py` builds the calibration/eval negatives from all cat images in Oxford-IIIT Pet, re-detecting with the same YOLO (shipped head-only boxes are ignored — see Data rules) into the same cache layout as Indy plus a `breed` column. Detector misses are not embedded (recoverable as catalog minus metadata).
- **Cat-breeds negatives (held-out FP stress test)** — `scripts/build_catbreeds_negatives.py` builds a second negatives source from the whole cat-breeds (Kaggle ma7555) dataset: kagglehub download, `--per-breed-limit` stratified seeded sample, corrupt-image skip, same variant-nested cache (`data/embeddings/catbreeds/<model>/<crop>/`) + sidecar + `breed` column as Oxford. It is **evaluation-only** — never enters the gallery or calibration (split discipline). `scripts/evaluate_catbreeds.py` (shim → `scripts/calibration/evaluate_catbreeds.py`) grades the frozen artifact against every cat-breeds cat as a negative with the Indy held-out `test` photos as positives, breaking out FPR on a 14-breed long-haired look-alike set and NFC specifically; text + HTML reports. Detail and the measured baseline are in `docs/calibration_design.md`. Tests: `tests/test_build_catbreeds.py`, `test_evaluate_catbreeds.py`, `test_evaluate_catbreeds_report_html.py`.
- **Decide** — `src/indycat/decision.py`: the UI-agnostic scoring core (`Gallery`, `aggregate`, `score`/`score_many`). Pure numpy, no I/O; choosing the threshold is calibration's job, not this module's. Tests: `tests/test_decision.py`.
- **Calibration & evaluation** — `scripts/calibration/` implements the decide-stage tooling: split manifests, role scoring, metrics, the frozen `calibration.yaml` artifact, the held-out evaluation grade, and text/HTML reports with screen-reader markup. Both calibrate and evaluate assert the cache variant is consistent (CLI request vs sidecars vs manifest/artifact) and fail loud on mismatch — the mechanics are in `docs/calibration_design.md`. Thin CLI shims: `scripts/calibrate.py`, `scripts/evaluate.py`. Tests: `tests/test_split_manifest.py`, `test_calibration_report.py`, `test_calibrate.py`, `test_artifact.py`, `test_evaluate.py`.
- **Shared helpers** — `scripts/_common.py`: image/IO helpers, the embedding/metadata helpers both gallery builders share, the cache reader (loud on row-count drift), and the embedding-provenance layer (variant slugs, the sidecar model and its `variant_key()` identity, read/write, `load_embeddings_variant`). Detail in `docs/embeddings_provenance.md`.
- **Data review app** — `scripts/data_review/` is a Streamlit app for inspecting the data (`uv run streamlit run scripts/data_review/data_review.py`): a mapping view (accessible "Rows" layout vs the literal `st.dataframe` grid, side by side so the screen-reader difference is felt), a crop-review view, and an Oxford detect-misses grid. Images are served via Streamlit static serving rather than inline base64 a screen reader would read out in full.
- **Prediction app** — `scripts/predict_app/` is the deliverable "Is it Indy?" Streamlit UI (`uv run streamlit run scripts/predict_app/app.py`): upload a photo, get a text-first verdict with detection confidence, crop, closest gallery match, and score-vs-threshold margin. `predict.py` is the streamlit-free core (`classify(...)`, handling the no-cat and multiple-cat edge cases). It wires the **frozen calibration artifact** (`data/artifacts/*.yaml`) into the live `Gallery` — never `data/embeddings/indy/`, so held-out test photos stay out of the live decision — and matches the live detect→crop→embed to the gallery's footing recorded in `artifact.embedding`. Tests: `tests/test_predict.py`.

torch is installed with CUDA from the pytorch-cu128 index (PyPI's Windows wheel is CPU-only — keep the `tool.uv` index pinning). Remaining decide-stage work is tracked in `docs/calibration_design.md` (notably the optional `leave_one_out` strategy and the held-out NFC look-alike slice).
...
Read `project_handoff.md` before making architectural decisions. Its decisions are described as a "considered starting point, not fixed requirements" — several are explicitly flagged as candidates for revision once real results come in (see its "Things most likely to change" section). Treat them as defaults to justify deviating from, not constraints.
...
**Apple Silicon (macOS):** runs without code changes. The PyTorch CUDA-index override in `pyproject.toml` is scoped to Windows (`sys_platform == 'win32'`), so macOS resolves the default PyPI torch wheel, which ships the MPS/Metal backend; `uv sync` then works as-is. `Embedder` auto-selects the device CUDA → MPS → CPU, so it uses the Apple GPU automatically. Rebuild the gallery embeddings on the Mac (the build scripts detect+embed on the fly) rather than copying CUDA-computed `.npy` files — cross-backend float rounding differs slightly. If either model hits an op not yet implemented on MPS, set `PYTORCH_ENABLE_MPS_FALLBACK=1` to route it to CPU.
